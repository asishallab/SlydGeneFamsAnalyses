...
option_list = list(make_option(c("-w", "--workDir"), type = "character", 
    ...
    metavar = "character"), make_option(c("-m", "--mercatorMapManAnnotations"), 
...
fams.meme.i <- which(unlist(lapply(fams.meme, function(x) !is.null(x))))

# Table of Slyd genes that are found to be subject of positive
# selection:
fams.meme.slyd.genes.df <- Reduce(rbind, lapply(names(fams.meme[fams.meme.i]), 
    function(g.f) {
    ...
    for (i in 1:nrow(fams.meme.slyd.genes.df)) {
        ...
        if (prot.id %in% mm4.tbl$IDENTIFIER) {
            mm4.tbl.prot <- mm4.tbl[which(mm4.tbl$TYPE & mm4.tbl$IDENTIFIER == 
                prot.id), ]
            fams.meme.slyd.genes.df$BINCODE[[i]] <- paste(mm4.tbl.prot$BINCODE, 
                collapse = ",")
            fams.meme.slyd.genes.df$NAME[[i]] <- paste(mm4.tbl.prot$NAME, 
                collapse = ",")
        }
    }
}
...
# Save results
save(fams.meme, fams.meme.i, fams.meme.slyd.genes.df, file = file.path(script.args$outDir, 
    "memeResults.RData"))
