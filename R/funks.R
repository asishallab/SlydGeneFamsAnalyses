...
parseOrthogroupsTxt <- function(path.2.orthogroups.txt) {
  ...
  fam.names <- unlist(lapply(fams.splt, function(x)
    x[[1]]))
...
#' already existed.
#' @export
createGeneFamilyWorkingDirIfNotExists <-
  function(base.dir, gene.family.name) {
...
#' @export
geneNameMappings <-
  function(gene.family.dir,
           gene.family.name,
           gene.fams.lst = gene.families) {
    gene.fam.name.maps.path <-
      file.path(gene.family.dir,
                paste0(gene.family.name,
                       "_name_mappings.txt"))
    if (file.exists(gene.fam.name.maps.path)) {
      ...
      gf.name.maps.df <-
        data.frame(original = gene.fams.lst[[gene.family.name]],
                   stringsAsFactors = FALSE)
      gf.name.maps.df$sanitized <-
        paste0("PROT", 1:length(gene.fams.lst[[gene.family.name]]))
      write.table(
        gf.name.maps.df,
        gene.fam.name.maps.path,
        row.names = FALSE,
...
mapOriginalToSanitizedNames <- function(originals, mappings) {
  ...
  unlist(lapply(originals, function(x)
    mappings[[x, "sanitized"]]))
}
...
mapSanitizedToOriginalNames <- function(sanitized, mappings) {
  ...
  unlist(lapply(sanitized, function(x)
    mappings[[x, "original"]]))
}
...
#' @export
loadAndSanitizeAAMsas <-
  function(path.2.msa.dir,
           working.dir,
           gene.families.names = names(gene.families)) {
    msa.fls <-
      system(paste("ls", path.2.msa.dir, "| grep -P \"^OG.*\\.fa$\""),
             intern = TRUE)
    names(msa.fls) <- sub("\\.fa$", "", msa.fls)
    if (!all(names(msa.fls) %in% gene.families.names)) {
      warning(
        "Some of the multiple amino acid sequence alignments' file names do not appear in the argument 'gene.families.names'."
      )
    }
    mclapply(names(msa.fls), function(gene.fam) {
      ...
      msa <-
        read.fasta(
          file.path(path.2.msa.dir, msa.file),
          seqtype = "AA",
          as.string = TRUE,
          strip.desc = TRUE
        )
      ...
      if (!all(names(msa) %in% names(all.cds))) {
        warning(
          "Some of the amino acid sequences in MSA '",
          msa.file,
          "' are not in the database of coding sequences ('all.cds')!"
        )
      }
      gene.fam.wd <-
        createGeneFamilyWorkingDirIfNotExists(working.dir,
                                              gene.fam)
      write.fasta(msa, names(msa), file.path(
        gene.fam.wd,
        paste0(gene.fam,
      ...
      names(msa) <-
        mapOriginalToSanitizedNames(names(msa), fam.gene.id.maps)
...
#' @return TRUE if and only if no error has occurred.
#' @export
loadAndSanitizeTrees <-
  function(path.2.trees.dir,
           working.dir,
           gene.families.names = names(gene.families)) {
    tree.fls <-
      system(paste("ls", path.2.trees.dir, "| grep -P \"^OG.*\\_tree.txt$\""),
             intern = TRUE)
    names(tree.fls) <- sub("\\_tree.txt$", "", tree.fls)
    if (!all(names(tree.fls) %in% gene.families.names)) {
      warning(
        "Some of the phylogenetic tree file names do not appear in the argument 'gene.families.names'."
      )
    }
    mclapply(names(tree.fls), function(gene.fam) {
      tree.file <- tree.fls[[gene.fam]]
      fam.tree <-
        read.tree(file.path(path.2.trees.dir, tree.file))
      fam.tree$tip.label <-
        sanitizeOrthofinderGeneIds(fam.tree$tip.label)
      if (!all(fam.tree$tip.label %in% names(all.cds))) {
        warning(
          "Some of the gene identifier in tree '",
          tree.file,
          "' are not in the database of coding sequences ('all.cds')!"
        )
      }
      ...
      fam.gene.id.maps <- geneNameMappings(gene.fam.wd, gene.fam)
      fam.tree$tip.label <-
        mapOriginalToSanitizedNames(fam.tree$tip.label,
                                    fam.gene.id.maps)
...
#' @return TRUE if and only if no error has occurred.
#' @export
generateCodingSequenceMSAs <-
  function(working.dir,
           gene.families.names = names(gene.families)) {
    gene.fams.dirs <-
      system(paste("ls", working.dir, "| grep -P \"^OG\""),
             intern = TRUE)
    if (!all(gene.fams.dirs %in% gene.families.names)) {
      warning(
        "Some of the gene families stored in working directory '",
        working.dir,
        "' do not appear in the argument gene.families.names!"
      )
    }
    mclapply(gene.fams.dirs, function(gene.fam) {
      ...
      fam.cds.msa <-
        GeneFamilies::alignCDSSetWithAlignedAAsAsGuide(fam.cds,
                                                       fam.aa.msa)
      fam.gene.id.maps <- geneNameMappings(gene.fam.wd, gene.fam)
      names(fam.cds.msa) <-
        mapOriginalToSanitizedNames(names(fam.cds.msa),
                                    fam.gene.id.maps)
      seqinr::write.fasta(fam.cds.msa,
                          names(fam.cds.msa),
                          file.path(gene.fam.wd,
...
#' @return TRUE if and only if no error has occurred.
#' @export
generateMemeBatchFiles <-
  function(working.dir,
           hyphy.batch.files.dir,
           gene.families.names = names(gene.families)) {
    ...
    mclapply(gene.fams.dirs, function(gene.fam) {
      ...
      fam.cds.msa.path <- file.path(gene.fam.wd, paste0(gene.fam,
                                                        "_CDS_MSA.fa"))
      fam.tree.no.node.labels.path <-
        file.path(gene.fam.wd, paste0(gene.fam,
                                      "_phyl_tree.newick"))
      fam.hyphy.meme.log.path <-
        file.path(gene.fam.wd, paste0(gene.fam,
                                      "_HyPhy_MEME_log.txt"))
      fam.hyphy.meme.output.path <-
        file.path(gene.fam.wd, paste0(gene.fam,
                                      "_HyPhy_MEME_output.txt"))
      fam.hyphy.meme.batch.file.path <-
        file.path(gene.fam.wd, paste0(gene.fam,
...
readMemeResults <-
  function(work.dir,
    ...
    fams.meme.outs <-
      system(paste("find", norm.wd, "-type f", "-name '*_HyPhy_MEME_output.txt'"),
             intern = TRUE)
    fams.meme.branches <- paste0(fams.meme.outs, ".branches")
    fam.nms <- sub("^.*/",
                   "",
                   sub("_HyPhy_MEME_output.txt$", "",
                       fams.meme.outs))
    ...
    setNames(mclapply(fam.nms, function(fam) {
      ...
      if (length(m.sites) > 0) {
        ...
        m.b <- read.table(
          fams.meme.branches[[fam]],
          sep = ",",
          header = TRUE,
          stringsAsFactors = FALSE,
          skip = 2
        )
        m.branches <- unique(m.b[which(
          m.b$Site %in% m.sites &
            m.b$EmpiricalBayesFactor >= empirical.bayes.factor.cutoff &
            grepl(phyl.tree.leaf.regex, m.b$Branch)
        ), c("Site",
             "Branch")])
        meme.branches.df <- if (nrow(m.branches) > 0) {
          ...
          if (report.original.gene.names) {
            ...
            fam.aa.msa.file <-
              normalizePath(file.path(fam.wd,
                                      paste0(
                                        fam, "_AA_MSA_orig_gene_ids.fa"
                                      )))
          } else {
          ...
          Reduce(rbind, lapply(m.b.unq, function(meme.branch) {
            ...
            Reduce(rbind, lapply(align.sites, function(align.site) {
              ...
              meme.site.p.adj <-
                m.sites.tbl[[which(m.sites.tbl$MEME.site ==
                                     align.site), "P.adj"]]
              data.frame(
                Protein = meme.branch,
                Gene.Family = fam,
                ...
                aligned.pos.sel.codon = align.site,
                MEME.site.p.adj = meme.site.p.adj,
                stringsAsFactors = FALSE
              )
            }))
          }))
...
#' @export
printAaMsaWithSelection <-
  function(fam.dir,
           aa.msa.file,
           selected.pos.and.genes.tbl,
           plot.pdf.prefix = sub(".fa", "", aa.msa.file, fixed = TRUE),
           before.sel.pos.offset = 5,
           after.sel.pos.offset = 5) {
    begin.codon <-
      min(selected.pos.and.genes.tbl$aligned.pos.sel.codon) -
      before.sel.pos.offset
    end.codon <-
      max(selected.pos.and.genes.tbl$aligned.pos.sel.codon) +
      after.sel.pos.offset
    m <-
      Biostrings::readAAMultipleAlignment(normalizePath(file.path(fam.dir,
                                                                  aa.msa.file)))
    aa.msa.prot.ids <- names(unmasked(m))
    gene.name.color.tex <-
      paste(unlist(lapply(selected.pos.and.genes.tbl$Protein,
                          function(sel.gene) {
                            i.gene <- which(aa.msa.prot.ids == sel.gene)
                            paste0("\\namecolor{", i.gene, "}{Magenta}")
                          })), collapse = "\n")
    
    frameblocks.emphregions.tex <-
      paste(unlist(lapply(selected.pos.and.genes.tbl$Protein,
                          function(sel.gene) {
                            gene.no <- which(aa.msa.prot.ids == sel.gene)
                            unalgnd.pos <-
                              sort(unique(selected.pos.and.genes.tbl[which(selected.pos.and.genes.tbl$Protein == sel.gene), "unaligned.pos.sel.codon"]))
                            framebl.tex <- paste0("\\frameblock{",
                                                  gene.no,
                                                  "}{",
    ...
    msa::msaPrettyPrint(
      ...
      output = "pdf",
      furtherCode = paste(gene.name.color.tex,
                          frameblocks.emphregions.tex, sep = "\n"),
      showNumbering = "none"
    )
  }
