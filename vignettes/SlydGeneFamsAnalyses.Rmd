...
```bash
grep -P '^>' Solanum_tuberosum_v3.4_cds.fa | sed -e 's/^>//' -e 's/ Protein//' -e 's/\t$//' > Solanum_tuberosum_v3.4_cds_ID_mappings.tsv
grep -P '^>' Solanum_tuberosum_v3.4_peptide.fa | sed -e 's/^>//' -e 's/ Protein.*$//' -e 's/\t$//' -e 's/\s\+/\t/' > Solanum_tuberosum_v3.4_peptide_ID_mappings.tsv
```

Then in an interactive R shell
```{r}
stu.cds.fa <- read.fasta('./Solanum_tuberosum_v3.4_cds.fa', seqtype='DNA', strip.desc=T, as.string=T )
...
### Load the material into this R package
